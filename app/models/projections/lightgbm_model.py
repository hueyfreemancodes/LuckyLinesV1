...
class LightGBMModel(BaseProjectionModel):
    ...
    def prepare_features(self, data: pd.DataFrame, team_stats: pd.DataFrame = None) -> tuple[pd.DataFrame, list]:
        ...
        col_ema = ['fantasy_points_ppr', 'targets', 'rush_attempts', 'red_zone_share', 'opportunity_share']
        df = FeatureEngineering.add_emas(df, span=4, cols=col_ema)
        
        col_lag = ['fantasy_points_ppr', 'targets', 'rush_attempts']
        df = FeatureEngineering.add_lags(df, lags=[1], cols=col_lag)
